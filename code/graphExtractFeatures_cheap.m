function X=graphExtractFeatures_cheap(CM3D,mod,classNames,cLabs,C_APPROACH,C_APPROACH_OPT,C_PLOT)
% 
% Extract features from graphs (cheap computing)
%
% IN
...
switch C_APPROACH
    ...
    case 'vprops' % vprops
        nProps=3; % how many properties we use
        ...
        for c=1:nClasses
            ...
            for s=1:nSubjects(c)
                ...
                %v_El=efficiency_wei(thisA,1);       % local efficiency - too expensive to compute
                v_bw=betweenness_wei(thisA);        % betweenness centrality
                embedThis=[v_s(:);v_CC(:);v_bw(:)];     % concatenate
                X(:,stOffset+s)=embedThis;
            end
            disp('');
            toc
