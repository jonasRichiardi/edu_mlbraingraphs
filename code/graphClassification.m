...
if exist('NaiveBayes','class')~=8
    tbcheck=ver('stats');
    if isempty(tbcheck)
        warning('Looks like you do not have the statistics toolbox. Naive Bayes will not work.');
    else
        warning('Looks like you have the statistics toolbox but Naive Bayes is not found on path. Naive Bayes will not work.');
    end
end

% FIXME use new syntax with R2015b and later, with parpool and gcp
% now check for parallel computing
if exist('matlabpool','file')==2
    ...
    warning(['You do not seem to have access to parallel computing. '...
        'The vertex property extraction code may run slowly, consider ' ...
        'using the _cheap version of the files.']);
end
...
C_DATASET='UCLA_Autism'; % 'Cambridge_youngOld', 'UCLA_Autism', 'UNIGE_MovieRest'
C_MOD=1;                                    % modality choice (for UCLA_Autism only). 1: fMRI, 2: DTI
C_BALANCECLASSES=true;
...
switch C_DATASET
    case 'UCLA_Autism'     
        ...
        load(fullfile('..','data','UCLA_Autism','CM3D.mat'));
        load(fullfile('..','data','UCLA_Autism','codebook.mat'));
    case 'UNIGE_MovieRest'
         mods={'fMRI'};                               % modality names
         classNames={'resting','movies'};
         load(fullfile('..','data','UNIGE_MovieRest','CM3D.mat'));
         load(fullfile('..','data','UNIGE_MovieRest','codebook.mat'));
    case 'Cambridge_youngOld'
         mods={'fMRI'};                               % modality names
         classNames={'y','o'};
         load(fullfile('..','data','Cambridge_youngOld','CM3D.mat'));
         load(fullfile('..','data','Cambridge_youngOld','codebook.mat'));        
end
...
C_APPROACH='dissimilarity';            % 'direct': direct embedding
                                % 'dissimilarity': dissimilarity embedding
                                % 'vprops': vertex properties
C_APPROACH_OPT='cosNorm';       % option for graph representation approach
...
X=graphExtractFeatures(CM3D,mods{C_MOD},classNames,cLabs,...
...
switch C_APPROACH
    ...
    case 'vprops'
        ...
        nProps=4;                   % if changing here must also change graphExtractFeatures
        pNames={'strength','clustering coefficient','local efficiency','betweenness centrality'};
